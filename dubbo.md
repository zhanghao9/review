...
Monitorfilter。 1. MonitorFilter 向 DubboMonitor 发送数据
...
提供方
配置
当 ProtocolConfig 和 ServiceConfig 某属性没有配置时，采用此缺省
值，可选
dubbo:co
nsumer/
消费方
配置 当 ReferenceConfig 某属性没有配置时，采用此缺省值，可选
dubbo:me
thod/
方法配
置
...
所以只希望加载某个的实现，就不现实了
DUBBO SPI： 1、对 Dubbo 进行扩展，不需要改动 Dubbo 的源码
