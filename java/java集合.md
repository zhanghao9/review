...
如果没有实现该接口，表示不支持 Random Access，如LinkedList。 Iterator<Integer> it = list.iterator(); while(it.hasNext()){ *// do something* it.remove(); } for(Integer i : list){ list.remove(i) }
